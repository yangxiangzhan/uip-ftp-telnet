...
struct avl_root shell_root = {.avl_node = NULL};//命令匹配的平衡二叉树树根 
...
void shell_tab(struct shell_buf * shellbuf)
{
    ...
    for (struct avl_node* node = avl_first(&shell_root); node ; node = avl_next(node))//遍历二叉树
	{
		...
		if (iFirstChar == CmdFirstChar)//首字母相同，匹配命令
		{
			...
			{
				match[match_cnt] = pshell_cmd;     //把匹配到的命令号索引记下来
				if (++match_cnt > 10) 
					return ;    //超过十条相同返回
			}
		}
	...
	else   //如果不止一条命令包含当前输入的字符串，打印含有相同字符的命令列表，并补全字符串输出直到命令区分点
	{
		...
		while(1)  //补全命令，把每条命令都包含的字符补全并打印
		{
			...
			shell_getchar(shellbuf,match[0]->pName[ucInputLen++]);  //把相同的字符补全到输入缓冲中
		}
	}
}
...
int shell_cmdparam(char * str,int * argc,int argv[])
{
	...
	for (ucCnt = 0; *str && ucCnt < 4; ++ucCnt)//字符不为 ‘\0' 的时候
	{
		argv[ucCnt] = 0;
